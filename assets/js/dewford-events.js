...
(function () {
  ...
  function connect() {
    ...
    var repeats = Math.max(5, Math.ceil(window.screen.width / (count * 368)) * 2 + 3);
    ...
    function position() {
      // Every preceding image remains 360px wide, plus an 8px gap.
      track.style.transform = 'translate3d(' + (container.clientWidth / 2 - current * 368 - 360) + 'px,0,0)';
    }
    function paint() {
    ...
    window.addEventListener('resize', function () { if (!moving) reset(); });
    reset();
    schedule();
  }
